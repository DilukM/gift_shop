...
export class AddItemToCartUseCase {
  ...
  async execute(product, quantity = 1) {
    try {
      ...
      const currentQuantity = await this.cartRepository.getItemQuantity(product.id);
...
export class UpdateCartItemQuantityUseCase {
  ...
  async execute(productId, quantity) {
    try {
      ...
      const cart = await this.cartRepository.updateQuantity(productId, quantity);
  ...
  async execute() {
    try {
      ...
      if (outOfStockItems.length > 0) {
        validationErrors.push(`Some items are out of stock: ${outOfStockItems.map(item => item.product.name).join(', ')}`);
      }
